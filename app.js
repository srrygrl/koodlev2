...
const authRegConfirmPassword = document.getElementById("authRegConfirmPassword");
...
// Roda antes de qualquer outra coisa — trava o app até logar
...
// ---------------- Alternar entre os formulários de entrar/criar conta ----------------
...
async function doLogin() {
  const identifier = authLoginUsername.value.trim();
  const password = authLoginPassword.value;
  authLoginError.textContent = "";

  if (!identifier || !password) {
    authLoginError.textContent = "Preenche usuário/e-mail e senha.";
    return;
  }

  try {
    const resp = await fetch(`${FRIENDS_API_BASE}/login`, {
      method: "POST",
    ...
    if (!resp.ok) {
      authLoginError.textContent = data.detail || "Usuário/e-mail ou senha incorretos.";
      return;
    }
    ...
    authLoginError.textContent = "Não consegui falar com o servidor. Ele está no ar?";
  }
}
...
async function doRegister() {
  ...
  const confirmPassword = authRegConfirmPassword.value;
  authRegisterError.textContent = "";

  if (!email || !username || !password || !confirmPassword) {
    authRegisterError.textContent = "Preenche todos os campos.";
    return;
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    authRegisterError.textContent = "Digita um e-mail válido.";
    return;
  }
  if (username.length < 3) {
    authRegisterError.textContent = "O nome de usuário precisa ter pelo menos 3 caracteres.";
    return;
  }
  if (password.length < 6) {
    authRegisterError.textContent = "A senha precisa ter pelo menos 6 caracteres.";
    return;
  }
  if (password !== confirmPassword) {
    authRegisterError.textContent = "As senhas não são iguais.";
    return;
  }

  try {
    const resp = await fetch(`${FRIENDS_API_BASE}/register`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (!resp.ok) {
      authRegisterError.textContent = data.detail || "Erro ao criar conta.";
      return;
    }
    ...
    authRegConfirmPassword.value = "";
    hideAuthGate();
    enterFriendsMainView();
  } catch {
    authRegisterError.textContent = "Não consegui falar com o servidor. Ele está no ar?";
  }
}
...
friendsLogoutBtn.addEventListener("click", async () => {
  ...
  // Volta pra tela de login (o login agora é obrigatório pra usar o app)
  authRegisterForm.classList.add("hidden");
  authLoginForm.classList.remove("hidden");
